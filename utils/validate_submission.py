...
Run this BEFORE submitting. It checks your output files against every
formatting rule the scorer enforces, so you can catch a rejection locally
instead of burning a submission. It reads only your output files and the
test source files (to learn which S1 entities are required and which
S2/S3 IDs exist); it never needs the ground truth and never computes your
score.
...
Exit code 0 means the files are safe to submit; 1 means fix the listed
issues (warnings never fail the run).
"""
    ...
    if check_ids:
        ...
        if valid_ids is not None:
            ...
            warnings.append(
                ...
                "candidate IDs exist in the test set. Re-run with --check-ids to enable."
            )
...
def main():
    ...
    try:
        ...
        print(
            ...
            f"{candidate_path}). Re-save as plain UTF-8, tab-separated .tsv."
        )
        return 1
